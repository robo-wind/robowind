...
namespace op
{
    ...
    struct PoseExtractorTensorRT::ImplPoseExtractorTensorRT
    {
            ...
            //std::shared_ptr<caffe::Blob<float>> spPoseBlob; // need to remove
        ...
        void addTensorRTNetOnThread(std::vector<std::shared_ptr<NetTensorRT>>& netTensorRT,
                                 ...
                                 const std::string& modelFolder, const bool enableGoogleLogging, const std::string tensorModel)
        {
            try
            {
                netTensorRT.emplace_back(
                                      std::make_shared<NetTensorRT>(modelFolder + getPoseProtoTxt(poseModel)+ tensorModel,
    ...
    void PoseExtractorTensorRT::netInitializationOnThread()
    {
        try
        {
                ...
                addTensorRTNetOnThread(upImpl->spTensorRTNets, upImpl->spTensorRTNetOutputBlobs, upImpl->mPoseModel,
                                    upImpl->mGpuId, upImpl->mModelFolder, upImpl->mEnableGoogleLogging, mtensorModel);
                                            ...
                                            const std::vector<double>& scaleInputToNetInputs)
    {
        ...
        try
        {
                ...
                while (upImpl->spTensorRTNets.size() < numberScales)
                    addTensorRTNetOnThread(upImpl->spTensorRTNets, upImpl->spTensorRTNetOutputBlobs, upImpl->mPoseModel,
                                        upImpl->mGpuId, upImpl->mModelFolder, false, "TENSORRT");
                ...
                //std::cout <<"GPU part="<< diff.count() << "ms" << std::endl;
                ...
                //std::cout <<"Connecting body parts="<< diff.count() << "ms" << std::endl;
    ...
    const float* PoseExtractorTensorRT::getCandidatesCpuConstPtr() const
    {
        try
        {
            #ifdef USE_CAFFE
    ...
    const float* PoseExtractorTensorRT::getCandidatesGpuConstPtr() const
    {
        try
        {
            #ifdef USE_CAFFE
    ...
    const float* PoseExtractorTensorRT::getPoseGpuConstPtr() const
    {
        try
        {
                ...
                checkThread();
                return nullptr;//upImpl->spPoseBlob->gpu_data();
            #else
                return nullptr;
            #endif
        }
