...
namespace op
{
    ...
    struct NetTensorRT::ImplNetTensorRT
    {
            ...
            {
                // std::cout << "net input size ????? " << mNetInputSize4D[0] << std::endl;
    ...
    ICudaEngine* NetTensorRT::caffeToGIEModel()
    {
        ...
        if (mEnableFP16 == true)
        {
            //check if fp16 is possible, if not regress to fp32 and print decision
            mEnableFP16 = (mOverride16 == true) ? false : builder->platformHasFastFp16();
            modelDataType = mEnableFP16 ? nvinfer1::DataType::kHALF : nvinfer1::DataType::kFLOAT;
        
            printf("*******************Platform %s FP16 support!!!*********************\n", mEnableFP16 ? "HAS" : "DOES NOT HAVE");
        }
        else
        {
            // do not check just force fp32 (default is fp32)
            modelDataType = nvinfer1::DataType::kFLOAT;
        }

        if (mEnableFP16 == true)
            printf("*******************Platform set to FP16 precision*******************\n");
        else
            printf("*******************Platform set to FP32 precision*******************\n");
    ...
    ICudaEngine* NetTensorRT::createEngine()
    {
        ...
        if (existFile(serializedEnginePath))
        {
            ...
            //std::cout<<"a is of type: "<<typeid(pluginFactory).name()<<std::endl;
            engine = infer->deserializeCudaEngine(gieModelStream, size, &pluginFactory);
    ...
    void NetTensorRT::initializationOnThread()
    {
        ...
        try
        {
                ...
                // std::cout << "NetInputSize4D: " 
                // << upImpl->mNetInputSize4D.at(0) << " " 
                // << upImpl->mNetInputSize4D.at(1) << " " 
                // << upImpl->mNetInputSize4D.at(2) << " " 
                // << upImpl->mNetInputSize4D.at(3) << std::endl;

                std::string net_size_input = upImpl->mCaffeProto.c_str();
                std::size_t idx_parser = net_size_input.find_last_of("_");
                net_size_input = net_size_input.substr(idx_parser+1);

                std::string delimiter = "x";
                std::string height_str = net_size_input.substr(0, net_size_input.find(delimiter));
                std::string width_str = net_size_input.substr(net_size_input.find(delimiter)+1, net_size_input.length());
               
                upImpl->spInputBlob = boost::make_shared<caffe::Blob<float>>(1, 3, std::stoi(height_str), std::stoi(width_str));
