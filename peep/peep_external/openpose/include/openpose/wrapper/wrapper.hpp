        ...
        try
        {
            ...
            if (numberThreads > 0)
            {
                ...
                if (wrapperStructPose.enable)
                {
                    ...
                    {
                        #ifdef USE_TENSORRT
                            const std::string tensor_model_file = "_" 
                                + std::to_string(wrapperStructPose.netInputSize.y) + "x"
                                + std::to_string(wrapperStructPose.netInputSize.x);
                            
                            poseExtractorNets.emplace_back(std::make_shared<PoseExtractorTensorRT>(
                ...
                else if (wrapperStructFace.enable)
                {
                ...
                if (wrapperStructFace.enable)
                {
                    ...
                    for (auto gpu = 0u; gpu < spWPoseExtractors.size(); gpu++)
                    {
                        ...
                        #ifdef USE_TENSORRT
                            const std::string tensor_model_file_face = "_" 
                                    + std::to_string(wrapperStructFace.netInputSize.y) + "x"
                                    + std::to_string(wrapperStructFace.netInputSize.x);
                            const auto faceExtractorNet = std::make_shared<FaceExtractorTensorRT>(
                                wrapperStructFace.netInputSize, netOutputSize, modelFolder, tensor_model_file_face,
                ...
                if (wrapperStructHand.enable)
                {
                    ...
                    for (auto gpu = 0u; gpu < spWPoseExtractors.size(); gpu++)
                    {
                        ...
                        #ifdef USE_TENSORRT
                            const std::string tensor_model_file_hand = "_" 
                                    + std::to_string(wrapperStructHand.netInputSize.y) + "x"
                                    + std::to_string(wrapperStructHand.netInputSize.x);
                            const auto handExtractorNet = std::make_shared<HandExtractorTensorRT>(
                                wrapperStructHand.netInputSize, netOutputSize, modelFolder, tensor_model_file_hand,
