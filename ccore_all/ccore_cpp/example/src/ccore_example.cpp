...
void
handle_test_msg(std::unique_ptr<ccore::msg_t> msg, void* callback_data)
{
    const char* result;
    BCON_EXTRACT(&msg->get_bson(), "data", BCONE_UTF8(result));
    std::cout << "1: " << msg->get_path() << ":" << result << std::endl;
}


void
handle_test_msg2(std::unique_ptr<ccore::msg_t> msg, void* callback_data)
{
    const char* result;
    BCON_EXTRACT(&msg->get_bson(), "data", BCONE_UTF8(result));
    std::cout << "2: " << msg->get_path() << ":" << result << std::endl;
}
...
main (int argc, char *argv [])
{
    ...
    client->add_pub("zmq-tcp://*:15555");
    client->add_sub("zmq-tcp://127.0.0.1:15555");
    ...
    client->subscribe("/test", handle_test_msg, NULL);
    client->subscribe("/test", handle_test_msg2, NULL);
